...
def extract_info(directory, prompt, output, markdown):
    """
    Extrae información relevante de archivos Excel usando un LLM a partir de un prompt, y guarda los datos en CSV.

    Args:
        directory (str): Ruta donde se encuentran los archivos Excel.
        prompt (str): Prompt de texto que indica qué información extraer.
        output (str): Archivo CSV donde se guardarán los datos extraídos.
        markdown (str): Archivo Markdown intermedio usado por el extractor.
    ...
    if os.path.exists(markdown):
        ...
        markdown_data = []
        excel_sheets = {}  # Mapeo de "archivo - hoja" a su ruta y nombre
        md_converter = MarkItDown()  # Conversor de Excel a Markdown
    ...
    if not is_valid_json(result):
        ...
        excel_sheets = {}  # Este diccionario debería venir del paso anterior
